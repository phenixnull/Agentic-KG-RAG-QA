...
if os.environ.get("RUNNING_IN_DOCKER") == "true":
    EMBED_MODEL_DIR = Path("/app/embedding")
else:
    PROJECT_ROOT = Path(__file__).resolve().parents[5]
    EMBED_MODEL_DIR = PROJECT_ROOT.parent / "refer_codebase" / "embedding"
...
DEFAULT_CHAT_MODEL_PROVIDERS: dict[str, ChatModelProvider] = {
    ...
    "fucheers": ChatModelProvider(
        name="Fucheers (Gemini)",
        url="https://www.fucheers.top",
        base_url="https://www.fucheers.top/v1",
        default="gemini-3-flash",
        env="FUCHEERS_API_KEY",
...
DEFAULT_EMBED_MODELS: dict[str, EmbedModelInfo] = {
    ...
    "local/bge-m3": EmbedModelInfo(
        model_id="local/bge-m3",
        name="bge-m3 (local)",
        dimension=1024,
        base_url=str(EMBED_MODEL_DIR / "bge-m3"),
        api_key="no_api_key",
        batch_size=16,
    ),
...
DEFAULT_RERANKERS: dict[str, RerankerInfo] = {
    ...
    "local/bge-reranker-v2-m3": RerankerInfo(
        name="BAAI/bge-reranker-v2-m3 (local)",
        base_url=str(EMBED_MODEL_DIR / "bge-reranker-v2-m3"),
        api_key="no_api_key",
    ),
