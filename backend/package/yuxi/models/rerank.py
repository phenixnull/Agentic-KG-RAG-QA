...
_LOCAL_RERANKER_CACHE: dict[str, tuple[object, object]] = {}
...
class LocalReranker(BaseReranker):
    ...
    def _build_payload(self, query: str, documents: list[str], max_length: int) -> dict[str, Any]:
        return {"query": query, "documents": documents, "max_length": max_length}
    ...
    def _ensure_loaded(self) -> None:
        ...
        with _LOCAL_RERANKER_CACHE_LOCK:
            cached = _LOCAL_RERANKER_CACHE.get(model_key)
            if cached is None:
                if not self.model_path.exists():
                    raise FileNotFoundError(f"Local reranker model path not found: {self.model_path}")

                logger.info(f"Loading local reranker model from {self.model_path}")
                ...
                _LOCAL_RERANKER_CACHE[model_key] = cached
    ...
    async def acompute_score(
        ...
        for start in range(0, len(documents), batch_size):
            ...
            outputs = await asyncio.to_thread(forward_once)
...
def get_reranker(model_id, **kwargs):
    ...
    if provider == "local":
        return LocalReranker(model_name=model_info.name, api_key=api_key, base_url=base_url, **kwargs)
