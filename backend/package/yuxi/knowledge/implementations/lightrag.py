...
class LightRagKB(KnowledgeBase):
    ...
    def _get_embedding_func(self, embed_info: dict):
        ...
        if config_dict.get("model_id", "").startswith("local/"):
            local_embedding_model = select_embedding_model(config_dict["model_id"])
            batch_size = int(getattr(local_embedding_model, "batch_size", 40) or 40)
            model_name = config_dict.get("name") or config_dict.get("model") or config_dict["model_id"]

            async def local_embedding_func(texts):
                vectors = await local_embedding_model.abatch_encode(texts, batch_size=batch_size)
