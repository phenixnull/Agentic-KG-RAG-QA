        ...
        if is_minio_url(item):
            ...
            if not content_hash:
                try:
                    ...
                    file_bytes = await get_minio_client().adownload_file(bucket_name, object_name)
                    content_hash = await calculate_content_hash(file_bytes)
                    logger.debug(f"Calculated content_hash for MinIO file: {item}")
                except Exception as exc:  # noqa: BLE001
                    logger.warning(f"Failed to calculate content hash for MinIO file {item}: {exc}")
                    raise ValueError(f"Missing content_hash for file: {item}") from exc
